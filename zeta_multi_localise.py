...
Extends ZETA inference for a single ECG to produce:
  ...
  - Multiple temporal localization intervals (from the cross-attention fusion path)
"""
...
DIFFUSE_ENTROPY_THRESHOLD = 0.88   
MIN_INTERVAL_TOKENS       = 3      # smallest meaningful interval (~96 ms)
PEAK_PERCENTILE           = 75     # neighbor expansion threshold percentile
...
def get_cross_attention_heatmap(model: M3AEModel,
    ...
    stacked = torch.stack(all_cross_attn, dim=0).mean(dim=0).squeeze(0) 
    stacked = stacked.mean(dim=0) 

    valid_text_len = int(text_mask.sum().item())
    ecg_timeline_heatmap = stacked[:valid_text_len, :].mean(dim=0) 

    heatmap = ecg_timeline_heatmap[1:]
    return heatmap.cpu()


def heatmap_to_intervals(heatmap: torch.Tensor):
    """
    Parses attention timeline for multiple high-density clusters.
    Returns: (list of (start_ms, end_ms), diffuse_flag)
    """
    ...
    # Noise filter to clear baseline text alignment bleed
    ...
    # Find tracking points hitting the top 90th percentile bounds
    strict_threshold = torch.quantile(h_filtered, 0.90).item()
    active_indices = (h_filtered >= strict_threshold).nonzero(as_tuple=True)[0].tolist()

    if not active_indices:
        return [], True

    # Group into separate contiguous chunks
    clusters = []
    current_cluster = [active_indices[0]]
...
def pick_dominant_lead(ecg: np.ndarray, intervals: list) -> int:
    if not intervals:
        return 0
    # Use the first valid interval cluster to isolate representative variance
...
def run(ecg: np.ndarray,
    ...
    # Process Multi-Interval Tracking
    def localise(text_seq, text_mask):
        heatmap = get_cross_attention_heatmap(
            model, text_seq, text_mask, ecg_seq, ecg_mask, device
        )
        intervals, diffuse = heatmap_to_intervals(heatmap)
        if diffuse or not intervals:
            return None, 0
        lead_idx = pick_dominant_lead(ecg, intervals)
        return intervals, lead_idx
    ...
    col_obs, col_scr, col_loc, col_bar = 45, 7, 30, 10
    header = f"  {'Observation':<{col_obs}}{'Score':>{col_scr}}  {'Localization':<{col_loc}}{'Strength'}"
    print(header)
    print("  " + "─" * (col_obs + col_scr + col_loc + col_bar + 20))

    def format_row(tag, text, score, loc_data, is_positive):
        intervals, lead_idx = loc_data
        if not intervals:
            loc_str = "diffuse"
        else:
            loc_str = ", ".join([f"{s}ms–{e}ms" for s, e in intervals[:2]])
            if len(intervals) > 2:
                loc_str += "..."
            loc_str += f" {LEAD_NAMES[lead_idx]}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ZETA localization for a single PTB-XL ECG")
